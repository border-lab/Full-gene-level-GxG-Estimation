...
- Heritability formula (p.12) is wrong: denominator should be $\sigma^2_g + \sigma^2_{g \times g} + \sigma^2_e$, not $\sigma^2_g + \sigma^2_{g \times g}$.
- Table of contents: all Results subsections point to "page 2."

## Framing

- The space complexity improvement ($O(nm)$ vs $O(n^2)$) is the strongest selling point. An $n \times n$ GRM can't even be stored at biobank scale. The abstract mentions $O(nm^2)$ time but buries the space result. Foreground it.
- The abstract and intro oversell. The method works under independence; for correlated SNPs (i.e., real data), SE doesn't decrease with n. Frame honestly: "works under independence, adjusted version removes bias for LD but consistency under strong LD remains open."
- The introduction doesn't engage with the counter-argument that statistical epistatic variance is expected to be small in outbred populations. If the quantity being estimated is near zero in humans, the SE requirements change.
