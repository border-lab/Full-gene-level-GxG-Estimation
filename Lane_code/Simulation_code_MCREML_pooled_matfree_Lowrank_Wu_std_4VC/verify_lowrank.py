...
dense kernels can be formed and compared directly:
  ...
  0. The NORMALIZATION DIVISOR -- the one thing this variant adds, and the
     THREE routes to it.  pooled_c_exact (the yardstick) is checked against the
     literal mean per-pair sample variance (written out here, pair by pair,
     sharing no code with it) and against the identity c = tr(P_c W_raw)/n that
     makes E[Var-hat(H gamma)] = c * s2gxg.  Both O(nm) plug-ins -- 'moment'
     (the pipeline's, C_METHOD; skewness from the empirical third moment) and
     'hwe' (the note's; skewness from the allele frequency) -- are checked
     against the literal pairwise sum of 1 + r_ab s_a s_b, also written out
     here, so the mat-vec algebra that collapses C(m_g,2) terms into one
     product is verified rather than assumed.  Then the NORMALIZED kernel is
     shown to have tr(P_c W)/n = c_exact / c-hat exactly -- the residual scale
     factor the plug-in leaves behind, NOT 1 -- and build_W_pooled's divisor is
     shown to be bit-identical to setup_pooled's: the simulation and the
     estimator must scale the kernel by the same number or the comparison means
     nothing.
  ...
  8. END TO END, THE POINT OF THE PIPELINE: averaged over many draws, the
     REALIZED variance of every component sits at its NOMINAL target -- with
     s2a = s2d = s2gxg = 0.1 and s2e = 0.7, the four realized variances centre
     on 0.1, 0.1, 0.1, 0.7.  The first three exactly; the epistasis one on
     (c_exact / c-hat) * s2gxg, which is what the z below is taken against, so
     the test measures the DRAWS and not the plug-in's known residual.  Checks
     0-2 prove the algebra; this one shows it on actual draws, and prints both
     where V_gxg would have landed without any normalization (c * s2gxg) and
     the residual the plug-in leaves.
"""
import argparse
import numpy as np
...
from Function_MCREML import (additive_design, dominance_design,
                             split_into_genes, build_W_pooled, pooled_c_exact,
                             pooled_c, compute_c_pooled, build_K, compute_KU,
                             hwe_skewness, third_moment_skewness,
                             setup_pooled, compute_WU_pooled,
                             simulate_Cholesky_4vc, simulate_phenotype,
                             mc_reml, R_DEFAULT, C_METHOD)
...
def pair_sum_W(genes, c):
    """Literal  W = (1/(P c)) sum_g sum_{a<b in g} h_ab h_ab',  h_ab = Z_a .* Z_b.

    The C(m_g, 2)-term sum written out, with no Hadamard identity anywhere --
    the yardstick build_W_pooled's closed form is measured against.  The
    normalization is applied here too (as an explicit argument, so the divisor
    under test is the one passed in), since it is part of the matrix the
...
def pair_sum_c(genes):
    """Literal  c = (1/P) sum_g sum_{a<b in g} Var-hat(Z_a .* Z_b),  ddof=0.

    The definition, one interaction column at a time, with none of
    pooled_c_exact's Gram-matrix algebra -- the yardstick the divisor is
    checked against.  O(n P), fine at the small sizes this script runs.
    """
    tot = 0.0
    P = 0
    for Zg in genes:
        ...
        for a in range(mg):
            for b in range(a + 1, mg):
                tot += float((Zg[:, a] * Zg[:, b]).var())
    return tot / P


def pair_sum_c_plugin(genes, s_all):
    """Literal  c-hat = (1/P) sum_g sum_{a<b in g} (1 + r_ab s_a s_b).

    The note's closed form written out ONE PAIR AT A TIME, with r_ab the sample
    correlation Z_a' Z_b / n computed per pair and none of the module's
    collapse of the double sum into s' R_g s = ||Z_g s||^2 / n.  This is the
    yardstick BOTH plug-ins' O(nm) algebra is measured against; s_all is the
    per-SNP skewness in column order, whichever route produced it.  O(n P),
    fine at the small sizes this script runs.
    """
    n = genes[0].shape[0]
    tot, P, k = 0.0, 0, 0
    for Zg in genes:
...
def exact_WU(genes, U, c):
    """Reference  W @ U  for the unstandardized c-normalized pooled kernel,
    O(n m_g^2) per column.
...
def main():
    ...
    # Draws for check 8's Monte-Carlo of the realized variances.  The se scales
    # as 1/sqrt(draws) and the quantity being checked is an exact identity, so
    # a few hundred is plenty to see a real bias; raise it to tighten the z.
    ap.add_argument('--mc_draws', type=int, default=400)
    ...
    # ---- 0. the normalization divisor c ----------------------------------
    # c must be (a) the literal mean per-pair sample variance and (b) equal to
    # tr(P_c W_raw)/n, the identity that makes E[Var-hat(H gamma)] = c * s2gxg.
    # Check 8 then confirms the consequence on actual draws.
    W_raw = np.zeros((n, n))
    P_all = 0
    for Zg in genes:
        mg = Zg.shape[1]
        ...
        P_all += mg * (mg - 1) // 2
        for a_ in range(mg):
            for b_ in range(a_ + 1, mg):
                h = Zg[:, a_] * Zg[:, b_]
                W_raw += np.outer(h, h)
    W_raw /= P_all
    c = pooled_c_exact(genes)
    c_pair = pair_sum_c(genes)
    Pc = np.eye(n) - np.ones((n, n)) / n
    c_trace = float(np.trace(Pc @ W_raw)) / n
    r0a = abs(c - c_pair) / c_pair
    r0b = abs(c - c_trace) / c_trace
    print(f"[0] exact divisor (the yardstick)  c = {c:.6f}")
    print(f"    vs literal mean per-pair Var-hat   rel = {r0a:.3e}   "
          f"{'OK' if r0a < 1e-10 else 'FAIL'}")
    print(f"    vs tr(P_c W_raw)/n                 rel = {r0b:.3e}   "
          f"{'OK' if r0b < 1e-10 else 'FAIL'}")

    # The two O(nm) plug-ins.  Each is the SAME closed form fed a different
    # skewness, so each is checked against the literal pairwise sum built from
    # ITS OWN s -- that isolates the mat-vec algebra (which must be exact) from
    # the approximation in s (which must not be).
    s_mom = np.concatenate([third_moment_skewness(g) for g in genes])
    s_hwe = hwe_skewness(SNP)
    c_mom = compute_c_pooled(SNP, a.G, method='moment')
    c_hwe = compute_c_pooled(SNP, a.G, method='hwe')
    for lab, c_hat, s_hat in (('moment (C_METHOD)', c_mom, s_mom),
                              ('hwe   (the note)', c_hwe, s_hwe)):
        lit = pair_sum_c_plugin(genes, s_hat)
        rel_alg = abs(c_hat - lit) / lit
        print(f"    {lab}  c-hat = {c_hat:.6f}   vs literal pairwise "
              f"rel = {rel_alg:.3e}   {'OK' if rel_alg < 1e-10 else 'FAIL'}"
              f"   |  vs exact c: rel gap = {abs(c_hat - c) / c:.3e}")
    print(f"    max |s_moment - s_hwe| = {np.max(np.abs(s_mom - s_hwe)):.4f}   "
          f"(the HWE departure this genotype has; RandomSNP is drawn UNDER "
          f"HWE, so it is sampling noise here and nothing more)")
    # C_METHOD must be the number the kernels actually use.
    c_used = pooled_c(genes)
    print(f"    pooled_c() default == compute_c_pooled(method={C_METHOD!r}): "
          f"{'OK' if c_used == c_mom else 'FAIL'}   -> the kernels divide by "
          f"c-hat = {c_used:.6f}, NOT by the exact c")

    # ---- 1. the closed form is the pair sum -----------------------------
    W, c_built = build_W_pooled(genes, return_c=True)
    r1 = _rel(W, pair_sum_W(genes, c_built))     # the divisor the kernel used
    print(f"[1] hadamard closed form vs pair sum  rel = {r1:.3e}   "
          f"{'OK' if r1 < 1e-10 else 'FAIL'}   (both c-normalized)")
    print(f"    build_W_pooled's own c matches pooled_c(): "
          f"{'OK' if c_built == c_used else 'FAIL'}")
    # The normalized kernel's OWN realized-variance factor.  With an exact
    # divisor it would be 1; with the O(nm) plug-in it is c_exact / c-hat, and
    # THAT is the identity to assert -- it is the residual scale the pipeline
    # carries, and asserting 1 would just be asserting the plug-in is perfect.
    c_norm_kernel = float(np.trace(Pc @ W)) / n
    c_resid = c / c_used
    r1c = abs(c_norm_kernel - c_resid) / c_resid
    print(f"    c of the NORMALIZED kernel, tr(P_c W)/n = {c_norm_kernel:.12f}"
          f"   vs c_exact/c-hat = {c_resid:.12f}   rel = {r1c:.3e}   "
          f"{'OK' if r1c < 1e-10 else 'FAIL'}")
    print(f"    => E[Var-hat(g_gxg)] = {c_resid:.6f} * s2gxg  (1.0 only if the "
          f"plug-in were exact; the gap is {abs(c_resid - 1.0) * 100:.3f}%)")

    # ---- 2. the independent exact reference == dense W ------------------
    U = rng.normal(size=(n, 3))
    r2 = _rel(exact_WU(genes, U, c_built), W @ U)
    ...
    for r in sorted({5, R_DEFAULT, r_full // 2, r_full}):
        F_lr, P, c_r = setup_pooled(genes, r=r)
        rel = _rel(compute_WU_pooled(genes, F_lr, P, c_r, U), ref)
        ...
        print(f"      r = {r:4d}   eig share = {share:.4f}   rel = {rel:.3e}"
              f"{exact}")
    F_lr, P, c_full = setup_pooled(genes, r=r_full)
    r3 = _rel(compute_WU_pooled(genes, F_lr, P, c_full, U), ref)
    print(f"      full rank (r = {r_full})               rel = {r3:.3e}   "
          f"{'OK' if r3 < 1e-10 else 'FAIL'}")
    print(f"      setup_pooled's c == build_W_pooled's c: "
          f"{'OK' if c_full == c_built else 'FAIL'}  (simulation and estimation "
          f"divide by the SAME number, bit for bit -- so the plug-in's own "
          f"error cancels between them)")
    u = U[:, 0]
    r3v = _rel(compute_WU_pooled(genes, F_lr, P, c_full, u),
               compute_WU_pooled(genes, F_lr, P, c_full, u.reshape(n, 1))[:, 0])
    ...
    F_tr, P, c_tr = setup_pooled(genes, r=a.r_reml)
    What = compute_WU_pooled(genes, F_tr, P, c_tr, np.eye(n))
    ...
    print(f"[6] tr(W) = {np.trace(W):.4f}  vs  tr(K_a) = {np.trace(Ka):.4f} "
          ...
          f"(an identity for K_a and K_d, NOT for W -- the c-normalization "
          f"fixes the realized-variance scale, not the trace)")
    ...
    La, Ld, Lgxg, _, c_sim = simulate_Cholesky_4vc(SNP, a.G, s2a=s2a, s2d=s2d,
                                                   s2gxg=s2gxg, s2e=s2e)
    assert c_sim == c_used, ("the simulation divided by a different c-hat "
                             "than pooled_c() returns")
    I = np.eye(n)
    Ks = [Ka, Kd, W, I]
    ...
    # ---- 8. the claim this pipeline exists for --------------------------
    # E[Var-hat(.)] = the NOMINAL target, for all four components at once.
    # Averaged over many draws from the SAME factors, each realized variance
    # must sit within a couple of standard errors of its target -- for the
    # epistasis component only because W carries the 1/c-hat.  Its target is
    # (c_exact / c-hat) * s2gxg, not s2gxg: the plug-in's residual is a KNOWN
    # constant here (check 1 measured it on the matrix), so folding it into the
    # target keeps this a test of the DRAWS.  Both the nominal target and where
    # V_gxg would land with no normalization at all (c * s2gxg) are printed
    # alongside, so the size of the correction and the size of what is left
    # over are both visible rather than asserted.
    print(f"[8] realized variance vs target over {a.mc_draws} draws "
          f"(the c-normalization's whole purpose):")
    np.random.seed(a.seed + 1)
    acc = np.zeros((a.mc_draws, 4))
    for j in range(a.mc_draws):
        _, vl, va, vd, ve = simulate_phenotype(La, Ld, Lgxg, n, s2a=s2a,
                                               s2d=s2d, s2gxg=s2gxg, s2e=s2e,
                                               return_realized=True)
        acc[j] = (va, vd, vl, ve)
    names = ('V_a  ', 'V_d  ', 'V_gxg', 'V_e  ')
    targets = (s2a, s2d, c_resid * s2gxg, s2e)
    for j, (nm, tgt) in enumerate(zip(names, targets)):
        col = acc[:, j]
        mean = col.mean()
        se = col.std(ddof=0) / np.sqrt(a.mc_draws)
        z = (mean - tgt) / se if se > 0 else 0.0
        print(f"    {nm}  target = {tgt:.4f}   realized mean = {mean:.4f} "
              f"+- {se:.4f} (se)   z = {z:+.2f}   "
              f"{'OK' if abs(z) < 3.0 else 'CHECK'}")
    # Both scale gaps, side by side: |c - 1| is what the RAW kernel carries,
    # |c_resid - 1| what is LEFT after dividing by the O(nm) c-hat.  Reported
    # as two numbers rather than a "% closed", which goes negative and reads
    # as a fault whenever the plug-in's O(1/sqrt n) noise happens to exceed an
    # already-small c - 1 (as it does on this LE genotype at small n).
    print(f"    (V_gxg's target above is (c_exact/c-hat) * s2gxg = "
          f"{c_resid * s2gxg:.4f} vs the nominal {s2gxg:.4f}.  With NO "
          f"normalization it would centre on c * s2gxg = {c * s2gxg:.4f}.  "
          f"Scale gap: raw |c - 1| = {abs(c - 1.0):.4f}, left after the "
          f"plug-in |c_exact/c-hat - 1| = {abs(c_resid - 1.0):.4f}.)")
