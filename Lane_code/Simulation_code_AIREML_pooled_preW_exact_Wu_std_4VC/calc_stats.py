...
come in ESTIMATE / REALIZED pairs:
  ...
  BOUNDARY REPLICATES.  ai_reml clamps components to [1e-9, 5 var(y)], and a
  ...
  documented in ai_reml).  Such rows are counted and named; --drop-boundary
...
# Components ai_reml can clamp to its lower bound (1e-9).  The threshold sits a
...
def boundary_rows(rows, labels):
    ...
    Returns (indices, per-label counts).  Only the components ai_reml actually
...
def summarise(filename, show_name=False, drop_boundary=False):
    ...
    if hits:
        ...
        if not drop_boundary:
            print("  Their OTHER components are biased too (see ai_reml's known "
