from Function_AIREML import *
import argparse
import os

parser = argparse.ArgumentParser()
parser.add_argument('--m', type=int, required=True)
parser.add_argument('--n', type=int, required=True)
parser.add_argument('--s2a', type=float, required=True)
parser.add_argument('--s2d', type=float, required=True)
parser.add_argument('--s2gxg', type=float, required=True)
parser.add_argument('--s2e', type=float, required=True)
parser.add_argument('--rep', type=int, required=True)
parser.add_argument('--mode', type=str, required=True)


args = parser.parse_args()

m = args.m
n = args.n
s2a = args.s2a
s2d = args.s2d
s2gxg = args.s2gxg
s2e = args.s2e
mode = args.mode
rep = args.rep


# Read genotype
SNP = pd.read_csv(f"/home/ziyanzha/MOM_within_gene/stored_genotype/{mode}SNP_n{n}_m{m}.csv", header=None)
SNP = SNP.to_numpy()

save_dir = "/home/ziyanzha/MOM_within_gene/AIREML/Cholesky_Lgxg"
save_path = f"{save_dir}/Lgxg_{mode}_n{n}_m{m}_s2a{s2a}_s2d{s2d}_s2gxg{s2gxg}_s2e{s2e}.npy"
Lgxg = np.load(save_path)

save_dir_s2a = "/home/ziyanzha/MOM_within_gene/AIREML/Cholesky_La"
save_path_s2a = f"{save_dir_s2a}/La_{mode}_n{n}_m{m}_s2a{s2a}_s2d{s2d}_s2gxg{s2gxg}_s2e{s2e}.npy"
La = np.load(save_path_s2a)

save_dir_s2d = "/home/ziyanzha/MOM_within_gene/AIREML/Cholesky_Ld"
save_path_s2d = f"{save_dir_s2d}/Ld_{mode}_n{n}_m{m}_s2a{s2a}_s2d{s2d}_s2gxg{s2gxg}_s2e{s2e}.npy"
Ld = np.load(save_path_s2d)

Z, y = simulate_remove_sampling_err(SNP, Lgxg, La, Ld, s2a=s2a, s2d=s2d, s2gxg=s2gxg, s2e=s2e)

output_dir = f"/home/ziyanzha/MOM_within_gene/AIREML/Phenotype/y_{mode}_n{n}_m{m}_s2a{s2a}_s2d{s2d}_s2gxg{s2gxg}_s2e{s2e}"

os.makedirs(output_dir, exist_ok=True)
y_path = f"{output_dir}/rep{rep}.csv"
pd.DataFrame(y).to_csv(y_path, index=False, header=False)


# Save Z only once (Z is deterministic for a given genotype matrix)
z_dir = "/home/ziyanzha/MOM_within_gene/stored_genotype"
z_path = f"{z_dir}/Z_{mode}_n{n}_m{m}.csv"
if not os.path.exists(z_path):
    os.makedirs(z_dir, exist_ok=True)
    pd.DataFrame(Z).to_csv(z_path, index=False, header=False)
    print(f"Z saved to: {z_path}")
