...
# Pooled-kernel counterpart of Simulate_Phenotype.py.
# Loads the pooled Lgxg/La (tagged by G/region_size/seed) and draws y.
# The Z file is identical to the single-gene pipeline (Z is deterministic for
# a given genotype matrix and independent of the pooling), so it is reused.
...
save_dir_s2a = "/home/ziyanzha/MOM_within_gene/AIREML_pooled/Cholesky_La"
save_path_s2a = f"{save_dir_s2a}/La_{mode}_n{n}_m{m}_s2a{s2a}_s2gxg{s2gxg}_s2e{s2e}_{tag}.npy"
La = np.load(save_path_s2a)

Z, y = simulate_remove_sampling_err(SNP, Lgxg, La, s2a, s2gxg, s2e)

output_dir = f"/home/ziyanzha/MOM_within_gene/AIREML_pooled/Phenotype/y_{mode}_n{n}_m{m}_s2a{s2a}_s2gxg{s2gxg}_s2e{s2e}_{tag}"
