...
# centering and no 1/sigma_ab scaling -- the SAME kernel the estimator applies
# matrix-free, which is the whole point of this directory (the standardized
# StochasticWu pipeline simulated one kernel and estimated another).
#
# The dense W is built here because a Cholesky factor needs an explicit matrix,
# but UNLIKE the _preW pipeline it is NOT cached: estimation rebuilds the action
# of W matrix-free from the genotype, so no n-by-n array is ever written to disk
# or loaded downstream.  Dropping the standardization also makes this build
# O(n^2 m) instead of O(n^2 m^2) -- the Hadamard identity replaces the pair loop
# -- so w_build_time here is far below the sibling's at the same (n, m, G).
Lgxg, w_build_time = simulate_Cholesky_gxg(SNP, G, s2gxg=s2gxg, s2e=s2e)

# Save the Cholesky factor (per variance target)
save_dir = "/home/ziyanzha/MOM_within_gene/MCREML_pooled_matfree_StochasticWu_unstd/Cholesky"
...
w_time_dir = "/home/ziyanzha/MOM_within_gene/MCREML_pooled_matfree_StochasticWu_unstd/time/result"
