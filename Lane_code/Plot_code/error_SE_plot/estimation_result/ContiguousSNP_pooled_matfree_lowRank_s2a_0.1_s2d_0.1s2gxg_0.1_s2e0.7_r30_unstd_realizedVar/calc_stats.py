...
pairwise-epistasis model, then Vl_hat = c_hat * s2gxg_hat (the c-corrected
estimate of the realized epistasis variance, realized_variance.pdf), then the
realized variances Var-hat(.) that this replicate's four effect draws actually
produced.
...
come in ESTIMATE / REALIZED pairs:
  ...
  O(1/sqrt n).  Reported for every pair present.  Note there is deliberately no
  pair for gxg: s2gxg_hat lives on the raw-H component scale and only Vl_hat =
  c_hat * s2gxg_hat is comparable to a realized variance.
