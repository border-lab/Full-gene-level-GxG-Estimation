# -*- coding: utf-8 -*-
"""
se_plot.py -- fixed-m relative-error (SE) box plot for the pooled-preW G-sweep
folders, written into result_figure/.
...
files themselves.  Truth (s2gxg, s2e) is still read from the folder name.
    ...
    python se_plot.py RandomSNP_pooled_preW_G1_s2gxg0.2_s2e0.8 [more folders...]
"""
import os
import re
import sys
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python se_plot.py <result_dir> [<result_dir> ...]")
        sys.exit(1)
    ...
    for arg in sys.argv[1:]:
        ...
        print(f"\nFolder     : {folder}")
        print(f"File base  : {file_basename}")
        ...
        # Name to match the existing G5/G10/G20 figures: folder tokens with the
        # real data type (RandomSNP -> chr1_10ksnp), written into result_figure/.
