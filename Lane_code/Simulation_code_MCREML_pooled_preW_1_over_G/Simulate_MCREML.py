...
# Load the PRE-COMPUTED pooled within-gene GRM (cached once by the Cholesky step)
...
w_path = f"/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/Wcache/W_pooled_{mode}_G{G}_n{n}_m{m}{rtag}{esttag}.npy"
if not os.path.exists(w_path):
    raise FileNotFoundError(
        f"estimation kernel not found: {w_path}\n"
        f"The Cholesky step must have been run with the SAME --ratio and "
        f"--estimate (rtag={rtag or '<none>'}, esttag={esttag or '<none>'}).")
W = np.load(w_path)
...
y_path = f"/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/Phenotype/y_{tag}/rep{rep}.csv"
...
output_dir = f"/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/result/{mode}_s2gxg{s2gxg}_s2e{s2e}_G{G}_n{n}m{m}{rtag}{esttag}"
...
time_dir = f"/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/time/rep_times/{mode}_s2gxg{s2gxg}_s2e{s2e}_G{G}_n{n}m{m}{rtag}{esttag}"
