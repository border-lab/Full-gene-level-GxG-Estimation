...
# SNPs in order.  Omit (or "even") for the equal split.  Note the ratio only
# sets which SNPs land in which gene; the 1/G kernel weights every gene equally.
...
rtag = ratio_tag(args.ratio)                # cache key, from the raw string
...
# simulate_Cholesky_pooled (several Z, one per gene) for the POOLED kernel.
...
# W_full = (1/G) sum_g K_g -- plus the GRMs themselves.  With --estimate, W_est
# is a second kernel pooled over the SELECTED genes only; that is what REML is
# fitted with, so the fit is deliberately misspecified while the truth stays the
# full panel.  Both kernels are deterministic per (genotype, split, subset), so
# they are PRE-COMPUTED and cached once and merely loaded at estimation time.
Lgxg, W, W_est, info = simulate_Cholesky_pooled(SNP, G, s2gxg=s2gxg, s2e=s2e,
...
print(f"estimation genes: {info['subset'] or 'all'}  "
      f"G_est/G_full = {info['gene_share']:.6f}")
...
# across variance settings and every replicate at estimation time).  The ratio
# tag keeps different gene splits from colliding, and the est suffix keeps
# different estimation subsets from colliding.
w_dir = "/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/Wcache"
os.makedirs(w_dir, exist_ok=True)
w_path = f"{w_dir}/W_pooled_{mode}_G{G}_n{n}_m{m}{rtag}.npy"
...
if W_est is not None:
    w_est_path = f"{w_dir}/W_pooled_{mode}_G{G}_n{n}_m{m}{rtag}{esttag}.npy"
    if not os.path.exists(w_est_path):
        np.save(w_est_path, W_est)
        print(f"W_est saved to: {w_est_path}")

# Record the gene split and the subset's share of GENES.  Because the truth is
# s2gxg * W_full but the fit spans only the subset's genes, the estimate is
# attenuated towards s2gxg * G_est/G_full -- keep that number with the run so
# the results can be read on either scale.
info_dir = "/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/info"
os.makedirs(info_dir, exist_ok=True)
info_name = f"{mode}_s2gxg{s2gxg}_s2e{s2e}_G{G}_n{n}m{m}{rtag}{esttag}"
with open(f"{info_dir}/split_{info_name}.txt", 'w') as f:
    ...
    f.write(f"G_full\t{info['G_full']}\n")
    f.write(f"G_est\t{info['G_est']}\n")
    f.write(f"gene_share\t{info['gene_share']:.10f}\n")
    f.write(f"s2gxg_true\t{s2gxg}\n")
    f.write(f"s2gxg_attenuated_target\t{s2gxg * info['gene_share']:.10f}\n")
...
w_time_dir = "/home/ziyanzha/MOM_within_gene/MCREML_pooled_preW_perG/time/result"
os.makedirs(w_time_dir, exist_ok=True)
fname = f"{mode}_s2gxg{s2gxg}_s2e{s2e}_G{G}_n{n}m{m}{rtag}{esttag}"
