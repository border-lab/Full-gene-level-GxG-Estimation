...
Contiguous= pd.read_csv("data/chr1_Contiguous1KSNP.raw", sep=r'\s+')
real_data = Contiguous.iloc[:, 6:].to_numpy()
real_data = real_data[:n, :m]

Lgxg, Le = simulate_Cholesky_from_std_sparse_pairVersion(real_data, s2gxg, s2e, stability=1e-8, density=density)  
Z, y = simulate_from_raw_only_W_remove_sampling_err(real_data, Lgxg, Le, s2gxg, s2e)
gxg, e, _ = MoM_only_M_std(Z, y, nmc=nmc)

filename = f"Contiguous_n{n}_m{m}_Pair_Density{density}.txt"  # Include density in filename
