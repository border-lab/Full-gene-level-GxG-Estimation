...
#   h_ab = Z_a.Z_b   (RAW product: no mean-centering, no 1/sigma_ab scaling),
...
#
# THE INTERACTION COLUMN IS UNSTANDARDIZED, on BOTH sides: build_W_pooled draws
# the phenotype from this W and compute_WU_pooled applies this W, so the two
# remain the same object (verified: the matrix-free apply reproduces the dense
# build to 5e-16).  Dropping the standardization collapses the per-gene pair
# weights to V = 1_{a!=b}, R = T = 0, so the centering vector v_R and scalar s_T
# are identically zero and setup no longer touches the genotype.  Two identities
# of the standardized kernel are given up with it: tr(W) = n and W 1 = 0, the
# latter meaning lambda_min(W) > 0 rather than 0.  Nothing in this pipeline
# relied on either.
#
# The MODEL, PHENOTYPE and ESTIMATOR are otherwise identical to _preW (which
# still uses the STANDARDIZED kernel -- a run here is no longer comparable to
# one there); only the way W is APPLIED differs:
#
...
DIR=/home/ziyanzha/MOM_within_gene/MCREML_pooled_matfree
...
JOB3=$(sbatch --parsable \
    ...
    --array=1-300%$ARRAY \
    --time=12:00:00 \
    $DIR/MCREML.sh $N $M $G $S2GXG $S2E $MODE $ITERS $NMC)
