...
#
# The PRECOMPUTED kernel W/W_<mode>_n<n>_m<m>_G<G>_<rtag>.npy is NOT removed:
# it does not depend on the variance targets, so other runs on the same
# genotype use the same matrix (their Cholesky jobs rewrite it identically).
# It is 8 n^2 bytes -- delete it by hand when a genotype is retired.
