...
# Load Za, Zd (additive / dominance designs, same files the pipeline saves)
...
y_path = f"/home/ziyanzha/MOM_within_gene/MCREML_two_var/Phenotype/y_{mode}_n{n}_m{m}_s2a{s2a}_s2d{s2d}_s2e{s2e}/rep{rep}.csv"
y = pd.read_csv(y_path, header=None).to_numpy().flatten()

# Monte-Carlo AI-REML: V = s2a K_a + s2d K_d + s2e I, GRMs built implicitly.
# seed=rep fixes the probe randomness per replicate (reproducible, yet varied
# across reps).
s2a_hat, s2d_hat, s2e_hat, _ = MC_REML(Za, Zd, y, iters=iters, nmc=nmc, seed=rep)

# Save result (s2a_s2d_s2e order)
output_dir = f"/home/ziyanzha/MOM_within_gene/MCREML_two_var/result/{mode}_n{n}m{m}_s2a{s2a}_s2d{s2d}_s2e{s2e}"
